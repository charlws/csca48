...
void related_ingredients(char source_ingredient[MAX_STR_LEN]) {
    ...
    int source_idx = ingredient_index(source_ingredient);
    if (source_idx == -1) return;
    for (int i = 0; i < MAT_SIZE; i++) {
        if (AdjMat[source_idx][i] > 0) {
            printf("%s\n", ingredients[i]);
        }
    }
}
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist) {
    ...
    if (k == dist) return h;
    // I think reducing k to 0 works better

    int sourceIdx = ingredient_index(source_ingredient);
    if(sourceIdx == -1) return h;

    // go from h->x to somewhere
    for (int i = 0; i < MAT_SIZE; i++) {
        if (AdjMat[sourceIdx][i] > 0 && !searchInt(h, i)) {
            h = insertInt(h, i);
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source,
                                   int k_avoid) {
    /*
    ...
    intNode *avoidHead = NULL;
    if(ingredient_index(avoid) != -1) avoidHead = insertInt(avoidHead, ingredient_index(avoid));
    avoidHead = related_k_dist(avoidHead, avoid, k_avoid, 0);

    intNode *current = head, *previous = NULL;
    while (current != NULL) {
        if (searchInt(avoidHead, current->x)) {
            // current is to be avoided
            if (previous == NULL) {
                // current is head
                head = current->next;
            } else {
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN]) {
     ...
     * Complete this function!
     ******/
    int allSourceIdx[10], toChangeIdx = -1;  // toChangeIdx is index in 'recipe'!
    for (int i = 0; i < 10; i++) {
        if (recipe[i][0] == '\0') {
            allSourceIdx[i] = -1;
            continue;
        }
        allSourceIdx[i] = ingredient_index(recipe[i]);
        if (strcmp(recipe[i], to_change) == 0) {
            toChangeIdx = i;
        }
    }

    int mostCompatibleIdx = -1, highestWeight = 0;  // mostCompatibleIdx is index in 'ingredients'
    for (int i = 0; i < 10; i++) {
        if (allSourceIdx[i] == -1) continue;
        const int sourceIdx = allSourceIdx[i];
        for (int j = 0; j < MAT_SIZE; j++) {
            if (AdjMat[sourceIdx][j] > highestWeight) {
                // more compatible?
                int ok = 1;
                for (int k = 0; k < 10; k++) {
                    if (allSourceIdx[k] == j) ok = 0;  // already in recipe
                }
                if (ok) {
                    highestWeight = AdjMat[sourceIdx][j];
                    mostCompatibleIdx = j;
                }
            }
        }
    ...
    if (mostCompatibleIdx != -1) strcpy(recipe[toChangeIdx], ingredients[mostCompatibleIdx]);
    // we don't do anything if we cannot find a more compatible ingredient
    // this can happen if MAT_SIZE = 10 and len(recipe) = 10
}
