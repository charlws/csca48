...
void caesar_cipher(unsigned char plaintext[MAX_STR_LEN], unsigned char passPic[50][50])
{
 ...
 //     tests it means you did not test carefully enough.
    ...
    while(plaintext[str_idx] != '\0'){
      int flag = (plaintext[str_idx] + passPic[pwd_idx / 50][pwd_idx % 50]) > 255;
      plaintext[str_idx] += passPic[pwd_idx / 50][pwd_idx % 50] + flag;
      pwd_idx++, str_idx++;
      if(pwd_idx == 2500) pwd_idx = 0;
    }
}
...
void caesar_decipher(unsigned char cryptotext[MAX_STR_LEN], unsigned char passPic[50][50])
{
 ...
 // TO DO: Implement this function
    ...
    while(cryptotext[str_idx] != '\0'){
      int flag = (cryptotext[str_idx] - passPic[pwd_idx / 50][pwd_idx % 50]) <= 0;
      cryptotext[str_idx] -= passPic[pwd_idx / 50][pwd_idx % 50] + flag;
      pwd_idx++, str_idx++;
      if(pwd_idx == 2500) pwd_idx = 0;
    }
}
