...
int main() {
    ...
    if (MDB_head == NULL) {
        ...
        if (strcmp(MDB_head->review.movie_title, "Alien") != 0 ||
            strcmp(MDB_head->review.movie_studio, "20th Century Fox") != 0 || MDB_head->review.year != 1979 ||
            MDB_head->review.BO_total != (float)203630630.0 || MDB_head->review.score != 85) {
    ...
    // Test #6 : Insert a near - duplicate Expects : Near duplicates are allowed, so a new node must be added,
    //     length should be 4 temp = MDB_head;
